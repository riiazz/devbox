...
| `configs`  | external service `env_file`s (v1.1) |
...
`configs/` holds the external environment files generated by
`devbox services add --env-file` and referenced by a service's `env_file`.
